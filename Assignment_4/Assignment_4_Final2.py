...
def calculate_AEP_sprogo(sector_dfs, Weib_parm_dic):
    ...
    for key, df in sector_dfs.items():
        ...
        for i in range(len(U) - 1): 
            ...
            pdf_value = weibull_pdf(U[i], k, A)
            pdfs.append(pdf_value)
            power_value = power_curve(U[i])
...
# Calculate u_star, then geostrophic wind, then new u_star and finally A parameter for WEIBULL !!
kappa = 0.4
z_0_water = 0.02e-2
z_0_land = 2e-2
z = 70
...
def gdl(U, z0, sector, k):
    ...
    # Compute new A parameter 
    A_new = u_star_final/sp.gamma(1+1/k)
...
def calculate_site_AEP(sector_dfs, sector_roughness, Weib_dic):   
    ...
    for sector, df in sector_dfs.items():
        ...
        for i in range(len(U) - 1): 
            ...
            A_new.append(gdl(midpoint_U, z0, sector, k))
            A_new_out.append(gdl(midpoint_U, z0, sector, k))
            
            pdf_value_new = weibull_pdf(midpoint_U, k, A_new[-1])
            power_value_new = power_curve(midpoint_U)
